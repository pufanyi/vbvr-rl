...
#   WORLD_SIZE=8 nodes => nodes 0-1 train, nodes 2-7 run rollout/reward actors.
...
if not string match -qr '^[1-9][0-9]*$' -- "$train_node_count"
    echo "ERROR: --train-nodes must be a positive integer, got $train_node_count" >&2
    exit 1
end
