...
# GT-duration-matched evaluation for the preconverted DiffSynth TI2V-5B
# step-35500 model. Each ground-truth duration is converted to a 16-FPS frame
# count and aligned to Wan's 4k+1 temporal contract. Generation stays at the
# checkpoint's native 512x512 resolution; every frame is then resized without
# crop to the GT canvas (the current 500-sample bench is uniformly 1024x1024)
# while preserving exact 16-FPS playback.
...
set -lx USE_ITEM_NUM_FRAMES 1
set -lx TEMPORAL_ALIGNMENT 4
set -lx INFER_FPS 16
...
or set -lx PREPARED_DIR $OUTPUT_ROOT/eval_gt_1024x1024_gt_duration_fps16
set -q SCORE_DIR[1]
or set -lx SCORE_DIR $OUTPUT_ROOT/scores

# The longest aligned sample is 281 / 16 = 17.5625 seconds. A loose duration
# ceiling keeps the preparer from raising the source 16-FPS rate.
set -lx MAX_DURATION 18
