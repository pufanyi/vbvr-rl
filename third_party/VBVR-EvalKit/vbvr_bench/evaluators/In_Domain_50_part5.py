...
class LightSequenceEvaluator(BaseEvaluator):
    ...
    def _evaluate_task_specific(
        ...
        matching_states = sum(1 for g, e in zip(gen_on_states, gt_on_states, strict=False) if g == e)
    ...
    def _evaluate_rule_understanding(self, gen_lights: list[dict], gt_lights: list[dict]) -> float:
        ...
        # Compare patterns
        matches = sum(1 for g, gt in zip(gen_pattern, gt_pattern, strict=False) if g == gt)
    ...
    def _evaluate_light_visual_quality(self, gen_frame: np.ndarray, gt_frame: np.ndarray,
        ...
        [l for l in gen_lights if not l.get('is_on', False)]
