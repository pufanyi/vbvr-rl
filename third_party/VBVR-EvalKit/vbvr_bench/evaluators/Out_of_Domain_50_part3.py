...
class OutlineInnermostSquareEvaluator(BaseEvaluator):
    ...
    def _evaluate_concentric_structure(
        ...
        for dist in distances:
            ...
            for fc, fnc in zip(first_colors, final_colors, strict=False):
                total += 1
                ...
                if is_blue:
                    ...
                    color_diff = sum(abs(int(c1) - int(c2)) for c1, c2 in zip(fc, fnc, strict=False))
                    if color_diff < 50:
                        matches += 1
...
class AddBordersToUnborderedEvaluator(BaseEvaluator):
    ...
    def _evaluate_scene_preservation(
        ...
        normalized_diff = color_diff / total if total > 0 else 0
