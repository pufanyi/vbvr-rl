...
from aoss_client.ceph.s3cpp.pys3client import (
    PyS3Client,
    S3Error,
    init_api,
    shutdown_api,
    ...
    def new_fn(self, cluster, bucket, key, *args, **kwargs):
        try:
            ...
            if err_type:
                ...
                new_err = exception.S3ClientError(err.error_name, err.error_message)
            else:
...
class S3CppClient(Ceph):
    ...
    def get_with_info(self, cluster, bucket, key, **kwargs):
        ...
        unsupported_ops = [k for k, v in kwargs.items() if k in ("enable_stream", "enable_etag") and v]
        if unsupported_ops:
