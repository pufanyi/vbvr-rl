...
from aoss_client.common.exception import (
    AccessDeniedError,
    NoSuchBucketError,
    NoSuchKeyError,
    RangeError,
...
class S3Client(Ceph):
    def __init__(self, cluster, conf, anonymous_access, *args, **kwargs):
        if anonymous_access:
            ...
            s3_args = {"aws_access_key_id": conf["access_key"], "aws_secret_access_key": conf["secret_key"]}
    ...
    def list(self, bucket, key, page_size=None):
        ...
        paging_args = {"Bucket": bucket, "Prefix": key, "Delimiter": "/", "PaginationConfig": {"PageSize": page_size}}
    ...
    def put_with_info(self, cluster, bucket, key, body, **kwargs):
        if isinstance(body, (bytes, bytearray)):
            if len(body) <= 1024 * 1024 * 1024 * 2:
                ...
                result, info = self.multipart_upload(cluster, bucket, key, BytesIO(body), **kwargs)
        elif hasattr(body, "read"):
            result, info = self.multipart_upload(cluster, bucket, key, body, **kwargs)
        else:
            raise TypeError(f"{type(self)} does not support content type {type(body)}")
    ...
    def multipart_upload(self, cluster, bucket, key, stream, chunk_size=1024 * 1024 * 1024 * 2, **kwargs):
        ...
        while True:
            ...
            parts.append({"PartNumber": part_id, "ETag": response["ETag"]})
            if enable_md5:
                md5.update(chunk)
    ...
    def s3_upload_file(self, cluster, filename, bucket, key, callback):
        assert self._cluster == cluster
        try:
            self._s3_client.upload_file(filename, bucket, key, Config=self._config, Callback=callback)
        except BotoClientError as err:
    ...
    def s3_download_file(self, cluster, filename, bucket, key, callback):
        assert self._cluster == cluster
        try:
            return self._s3_client.download_file(bucket, key, filename, Config=self._config, Callback=callback)
        except BotoClientError as err:
            if err.response["ResponseMetadata"]["HTTPStatusCode"] == 404:
                raise NoSuchKeyError(cluster, bucket, key, err) from None
            elif err.response["ResponseMetadata"]["HTTPStatusCode"] == 403:
    ...
    def s3_download_fileobj(self, cluster, filename, bucket, key, callback):
        assert self._cluster == cluster
        try:
            return self._s3_client.download_fileobj(bucket, key, filename, Config=self._config, Callback=callback)
        except BotoClientError as err:
            if err.response["ResponseMetadata"]["HTTPStatusCode"] == 404:
                raise NoSuchKeyError(cluster, bucket, key, err) from None
            elif err.response["ResponseMetadata"]["HTTPStatusCode"] == 403:
    ...
    def generate_presigned_url(self, cluster, bucket, key, client_method, expires_in):
        ...
        return self._s3_resource.meta.client.generate_presigned_url(
            client_method, {"Bucket": bucket, "Key": key}, expires_in
        )
    ...
    def generate_presigned_post(self, cluster, bucket, key, fields=None, conditions=None, expires_in=3600):
        ...
        return self._s3_resource.meta.client.generate_presigned_post(bucket, key, fields, conditions, expires_in)
