...
def display_top(snapshot: Snapshot, limit=10, buffer=None, key_type="lineno"):
    ...
    if other:
        ...
        write(f"Other:\t{len(other)} items, {other_blocks} blocks, {format_size(other_size)}")

    total_size = sum(stat.size for stat in stats)
    total_blocks = sum(stat.count for stat in stats)
    write(f"Total:\t{len(stats)} items, {total_blocks} blocks, {format_size(total_size)}")
...
def filter_traces(snapshot, pattern):
    return snapshot.filter_traces((tracemalloc.Filter(True, pattern),))
