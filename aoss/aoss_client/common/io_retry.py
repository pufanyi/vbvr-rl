...
def _retry(op_name, exceptions, raises, tries, fn, client, *args, **kwargs):
    ...
    for count in range(1, retry_max + 1):
        try:
            ...
            if count < retry_max:
                LOG.debug(
                    "Exception occurred in the %s retry of %s operation on (%s): %s", ordinal(count), op_name, uri, err
                )
                continue
            if retry_max > 1:
                LOG.error(
                    "%s operation (%s) has tried %s times and failed: %s", op_name.capitalize(), uri, retry_max, err
                )
            raise
