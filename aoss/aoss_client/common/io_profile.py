...
class StatItem(object):
    ...
    @property
    def time_avg(self):
        return self.total_time / self.total_io if self.total_io else 0.0

    @property
    def hit_ratio(self):
        return 1.0 * self.total_hit / self.total_io if self.total_io else 0.0

    @property
    def speed(self):
        return 1.0 * self.total_byte / self.total_time if self.total_time else 0.0

    def stat_io(self, callback=None):
        stat_info = (
            f"{self.op_name} [total: {self.total_io}"
        ...
        if self.error_count:
            items = ["{}: {}".format(k, v) for (k, v) in self.error_count.items()]
...
class Profiler(object):
    ...
    def __init__(self, conf, *args, **kwargs):
        ...
        self.op_count = 0
        self.count_disp = ENV.int("count_disp", None) or conf.get_int("count_disp")
    ...
    def set_count_disp(count_disp):
        if count_disp < 0:
            LOG.error("count_disp must be a nonnegative integer, actual value: %s", count_disp)
            return
    ...
    def stat_io(self):
        if LOG.isEnabledFor(logging.INFO):
            io_dict = {client_stat.name: client_stat.get_hit for client_stat in self.stat_dict.values()}
            total_io = sum(io_dict.values()) or 1
            percentage = [f"{client_name}: {1.0 * count / total_io :.2%}" for client_name, count in io_dict.items()]
